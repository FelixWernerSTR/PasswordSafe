<template>
  <div>
    <h2 id="page-heading" data-cy="EntryHeading">
      <span v-text="$t('passwortverwaltungApp.entry.home.title')" id="entry-heading">Entries</span>
      <div class="d-flex justify-content-end">
        <button class="btn btn-info mr-2" v-on:click="handleSyncList" :disabled="isFetching">
          <font-awesome-icon icon="sync" :spin="isFetching"></font-awesome-icon>
          <span v-text="$t('passwortverwaltungApp.entry.home.refreshListLabel')">Refresh List</span>
        </button>
        <router-link :to="{ name: 'EntryCreate' }" custom v-slot="{ navigate }">
          ...
          >
            ...
            <span v-text="$t('passwortverwaltungApp.entry.home.createLabel')"> Create a new Entry </span>
          </button>
        </router-link>
      </div>
    </h2>
    ...
    <div class="alert alert-warning" v-if="!isFetching && entries && entries.length === 0">
      <span v-text="$t('passwortverwaltungApp.entry.home.notFound')">No entries found</span>
    </div>
        ...
        <thead>
          <tr>
            ...
            <th scope="row" v-on:click="changeOrder('description')">
              <span v-text="$t('passwortverwaltungApp.entry.description')">Description</span>
              <jhi-sort-indicator :current-order="propOrder" :reverse="reverse" :field-name="'description'"></jhi-sort-indicator>
            </th>
            <th scope="row" v-on:click="changeOrder('login')">
              <span v-text="$t('passwortverwaltungApp.entry.login')">Login</span>
            ...
            <th scope="row" v-on:click="changeOrder('passwort')">
              <span v-text="$t('passwortverwaltungApp.entry.passwort')">Passwort</span>
            ...
            <th scope="row" v-on:click="changeOrder('passwortReplay')">
              <span v-text="$t('passwortverwaltungApp.entry.passwortReplay')">Passwort Replay</span>
            ...
            <th scope="row" v-on:click="changeOrder('group.name')">
              <span v-text="$t('passwortverwaltungApp.entry.group')">Group</span>
    ...
    <b-modal ref="removeEntity" id="removeEntity">
      <span slot="modal-title"
        ><span id="passwortverwaltungApp.entry.delete.question" data-cy="entryDeleteDialogHeading" v-text="$t('entity.delete.title')"
          >Confirm delete operation</span
        ></span
      >
      <div class="modal-body">
        <p id="jhi-delete-entry-heading" v-text="$t('passwortverwaltungApp.entry.delete.question', { id: removeId })">
