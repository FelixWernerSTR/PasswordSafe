...
class BeitragResourceIT {
    ...
    void createBeitrag() throws Exception {
        ...
        // Create the Beitrag
        BeitragDTO beitragDTO = beitragMapper.toDto(beitrag);
        restBeitragMockMvc
            .perform(
                post(ENTITY_API_URL)
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(beitragDTO))
            )
    ...
    void putNewBeitrag() throws Exception {
        ...
        updatedBeitrag
            ...
            .publishDate(UPDATED_PUBLISH_DATE)
            .archiv(UPDATED_ARCHIV);
        BeitragDTO beitragDTO = beitragMapper.toDto(updatedBeitrag);

        restBeitragMockMvc
            .perform(
                put(ENTITY_API_URL_ID, beitragDTO.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(beitragDTO))
            )
    ...
    void patchNonExistingBeitrag() throws Exception {
        ...
        restBeitragMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, beitragDTO.getId())
                    .with(csrf())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(beitragDTO))
            )
