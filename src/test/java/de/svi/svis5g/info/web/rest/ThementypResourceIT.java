...
class ThementypResourceIT {
    ...
    @Autowired
    private ThementypMapper thementypMapper;
    ...
    void createThementyp() throws Exception {
        ...
        ThementypDTO thementypDTO = thementypMapper.toDto(thementyp);
        restThementypMockMvc
            .perform(
                post(ENTITY_API_URL)
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(thementypDTO))
            )
    ...
    void putNewThementyp() throws Exception {
        ...
        ThementypDTO thementypDTO = thementypMapper.toDto(updatedThementyp);

        restThementypMockMvc
            .perform(
                put(ENTITY_API_URL_ID, thementypDTO.getId())
                    .with(csrf())
                    .contentType(MediaType.APPLICATION_JSON)
                    .content(TestUtil.convertObjectToJsonBytes(thementypDTO))
            )
    ...
    void patchNonExistingThementyp() throws Exception {
        ...
        restThementypMockMvc
            .perform(
                patch(ENTITY_API_URL_ID, thementypDTO.getId())
                    .with(csrf())
                    .contentType("application/merge-patch+json")
                    .content(TestUtil.convertObjectToJsonBytes(thementypDTO))
            )
