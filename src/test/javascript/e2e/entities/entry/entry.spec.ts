...
describe('Entry e2e test', () => {
  ...
  describe('Create flow', () => {
    ...
    it('should create and save Entries', async () => {
      ...
      expect(await updatePage.loginInput.getAttribute('value')).to.match(/login/);

      await updatePage.passwortInput.sendKeys('passwort');
      expect(await updatePage.passwortInput.getAttribute('value')).to.match(/passwort/);

      await updatePage.passwortReplayInput.sendKeys('passwortReplay');
      expect(await updatePage.passwortReplayInput.getAttribute('value')).to.match(/passwortReplay/);
    ...
    describe('Details, Update, Delete flow', () => {
      ...
      it('should load edit Entry page, fetch data and update', async () => {
        ...
        expect(await updatePage.loginInput.getAttribute('value')).to.match(/modified/);

        await updatePage.passwortInput.clear();
        await updatePage.passwortInput.sendKeys('modified');
        expect(await updatePage.passwortInput.getAttribute('value')).to.match(/modified/);

        await updatePage.passwortReplayInput.clear();
        await updatePage.passwortReplayInput.sendKeys('modified');
        expect(await updatePage.passwortReplayInput.getAttribute('value')).to.match(/modified/);
